...
afw_player_selections_lockout <- read_parquet(paste0("https://github.com/bit-in-that/data-automation/raw/main/aflw_fantasy/data/raw/afw_player_selections_lockout_round", parameters$round_number -1 , ".parquet"))
...
Note that the selections below are as at the end of round `r parameters$round_number -1 ` (trades since then are not included).
...
live_selections |> 
  reactable(
    columns = list(
      ...
      selections_round_end = colDef(name = paste0("Round ", parameters$round_number -1 ), format = colFormat(digits = 2, percent = TRUE), width = 100),
...
The tables below shows the cheapest on-field player in each line as an indication of how "complete" people's teams are as at the end of round `r parameters$round_number -1 `.
