...
The tables below shows the cheapest on-field player in each line as an indication of how "complete" people's teams are.
...
top_10000_worst_players <- top_10000_lineups |> 
  ...
  group_by(id, full_name, player_position, current_price, starting_price, avgPoints) |> 
  summarise(
  ...
  top_10000_worst_players_position |> 
    reactable(
      columns = list(
        ...
        avgPoints = colDef(name = "Average", width = 75),
        ...
        top_100 = colDef(name = "100🧢", format = colFormat(digits = 2, percent = TRUE), width = 80),
        first = colDef(name = "First🚗", format = colFormat(digits = 2, percent = TRUE), width = 80),
        id = colDef(show = FALSE)
      ),
      columnGroups = list(
