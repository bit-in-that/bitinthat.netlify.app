---
title: "Supercoach Player Selections"
execute:
  echo: false
  warning: false
---
...
player_selections_plot <- read_parquet("https://github.com/bit-in-that/data-automation/raw/main/2026/output/sc_player_selections_long.parquet") |> 
    mutate(
  ...
  player_selections_table |> 
    reactable(
      columns = list(
        ...
        ownership = colDef(name = "Official", format = colFormat(digits = 1, suffix = "%")),
        ownership_adjusted_now = colDef(name = "Adjusted", format = colFormat(digits = 1, suffix = "%")),
        ownership_max = colDef(show = FALSE),
        ownership_adjusted_max = colDef(show = FALSE),
        ownership_adjusted_max_date = colDef(show = FALSE),
        change_yesterday = colDef(name = "Yesterday", format = colFormat(digits = 1, suffix = "%"),
          style = \(value) list(color = if (is.na(value) || is.nan(value)) "gray" else if (value >= 0) "green" else "red")),
        change_week = colDef(name = "Past Week", format = colFormat(digits = 1, suffix = "%"),
          style = \(value) list(color = if (is.na(value) || is.nan(value)) "gray" else if (value >= 0) "green" else "red")),
        change_max = colDef(show = FALSE),
        change_max_date = colDef(show = FALSE),
        percent_change_yesterday = colDef(name = "Yesterday %", format = colFormat(percent = TRUE, digits = 1),
          style = \(value) list(color = if (is.na(value) || is.nan(value)) "gray" else if (value >= 0) "green" else "red")),
        percent_change_week = colDef(name = "Past Week %", format = colFormat(percent = TRUE, digits = 1),
          style = \(value) list(color = if (is.na(value) || is.nan(value)) "gray" else if (value >= 0) "green" else "red")),
        gradient = colDef(show = FALSE)
      ),
      columnGroups = list(
        colGroup(name = "Current Ownership", columns = c("ownership", "ownership_adjusted_now")),
        colGroup(name = "Change Yesterday", columns = c("change_yesterday", "percent_change_yesterday")),
        colGroup(name = "Change Past Week", columns = c("change_week", "percent_change_week"))
      ),
      filterable = TRUE,
      defaultPageSize = 10,
      pagination = TRUE,
      elementId = "sc-player-selections-table"
    ),
  tags$div(
    style = "text-align: center; margin-top: 10px;",
    tags$script(HTML("var currentPageSize = 10;")),
    tags$button(
      "Show 10 more",
      style = "padding: 8px 20px; cursor: pointer; border: 1px solid #ced4da; border-radius: 4px; background: #f8f9fa;",
      onclick = "
        currentPageSize += 10;
        Reactable.getInstance('sc-player-selections-table').setPageSize(currentPageSize);
      "
    )
  )
)
...
When people look at the ownership percentages for Supercoach, they often wonder why the most selected players are all below 70% (people usually assume this is because there are a lot of casual or autofill teams out there). The reason for this is that a lot of teams are either missing players or are entirely empty. Fortunately there is a solution to this, I have devised the following formula to calculate a player's true selected number:
...
$$
(\text{Overall Team Completion \%}) = \frac{ \sum ( \text{\% Ownership})}{  31 }
$$


Note that this is a rough approximation but makes it easier to compare "apples with apples" when plotting ownership percentages over time.
...
update_plotly_plot_ownership = function(player_id) {
  ...
  fetch("json_data/sc_adjusted_plot_data/" + player_id + ".json")
    ...
    .then(data_player => {
      ...
      if (decreaseIndices.length) {
        Plotly.restyle(plotId, {
          ...
          y: [data_player['daily_decrease_y']],
          base: [data_player['daily_decrease_x'].map(() => 0)],  // reset base to zeros
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if(playerId) {
    ...
    update_plotly_plot_ownership("693"); //Bont
