  ...
  fetch("json_data/sc_adjusted_plot_data/" + player_id + ".json")
    ...
    .then(data_player => {
      ...
      if (decreaseIndices.length) {
        // Create new y array with every second index using daily_decrease_y
        const yValues = data_player['daily_decrease_y_abs'].map((val, idx) => 
          idx % 2 === 1 ? data_player['daily_decrease_y'][idx] : val
        );
