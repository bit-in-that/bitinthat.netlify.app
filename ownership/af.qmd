...
tagList(
  player_selections_table |> 
    reactable(
      ...
      defaultPageSize = 10,
      pagination = TRUE,
      elementId = "player-selections-table"
    ),
  tags$div(
    ...
    tags$button(
      ...
      onclick = "
        currentPageSize += 10;
        var instance = Reactable.getInstance('player-selections-table');
...
When people look at the ownership percentages for AFL fantasy, they often wonder why the most selected players are all below 70% (people usually assume this is because there are a lot of casual or autofill teams out there). The reason for this is that a lot of teams are either missing players or are entirely empty. Fortunately there is a solution to this, I have devised the following formula to calculate a player's true selected number:
...
update_plotly_plot_ownership = function(player_id) {
  ...
  fetch("json_data/af_adjusted_plot_data/" + player_id + ".json")
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if(playerId) {
    ...
    update_plotly_plot_ownership("1006130"); //Baily Smith
