...
player_selections_modified <- player_selections_long |> 
  mutate(
    ...
    text_snapshot = paste0("Snapshot Date: ", format(snapshot_date, format = "%Y-%m-%d"), "<br />Owned By  % (Adjusted): ", round(owned_by_adjusted, 2), "<br />Owned By  % (Official): ", round(owned_by, 2), "<br /># Coaches: ", fantasy_coaches, "(", round(completion_percentage, 1), "% complete)"),
    text_daily_change = paste0("Snapshot Date: ", format(snapshot_date, format = "%Y-%m-%d"), "<br />Daily Movement In Adjusted Ownership: ", round(owned_by_adjusted_diff, 2), "<br /># Coaches: ", fantasy_coaches, " (", round(completion_percentage, 1), "% complete)"),
    )
